...
class SVDGroupNorm32(SVDGroupNorm):
    def forward(self, x):
        return super().forward(x.float()).type(x.dtype)
    
def SVDnormalization(channels):
    """
    Make a standard normalization layer.
    :param channels: number of input channels.
    :return: an nn.Module for normalization.
    """
    return SVDGroupNorm32(32, channels)

def SVDconv_nd(dims, *args, **kwargs):
    """
...
## go
class SVDAttentionPool2d(nn.Module):
    """
    ...
    def __init__(
        ...
        self.qkv_proj = SVDconv_nd(1, embed_dim, 3 * embed_dim, 1)
        self.c_proj = SVDconv_nd(1, embed_dim, output_dim or embed_dim, 1)
...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, padding=1):
        ...
        self.dims = dims
        if use_conv:
            self.conv = SVDconv_nd(dims, self.channels, self.out_channels, 3, padding=padding)
...
class SVDResBlock(TimestepBlock):
    """
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            SVDnormalization(channels),
            nn.SiLU(),
            SVDconv_nd(dims, channels, self.out_channels, 3, padding=1),
        )
        ...
        self.out_layers = nn.Sequential(
            SVDnormalization(self.out_channels),
            nn.SiLU(),
            nn.Dropout(p=dropout),
            zero_module(
                SVDconv_nd(dims, self.out_channels, self.out_channels, 3, padding=1)
            ),
        )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = SVDconv_nd(
                dims, channels, self.out_channels, 3, padding=1
            )
        else:
            self.skip_connection = SVDconv_nd(dims, channels, self.out_channels, 1)
...
class SVDAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.norm = SVDnormalization(channels)
        self.qkv = SVDconv_nd(1, channels, channels * 3, 1)
        ...
        self.proj_out = zero_module(SVDconv_nd(1, channels, channels, 1))
...
class SVDUNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.time_embed = nn.Sequential(
            SVDlinear(model_channels, time_embed_dim),
            nn.SiLU(),
            SVDlinear(time_embed_dim, time_embed_dim),
        )

        if self.num_classes is not None:
            self.label_emb = SVDEmbedding(num_classes, time_embed_dim)

        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    SVDconv_nd(dims, in_channels, model_channels, 3, padding=1)
                )
            ]
        )
        ...
        self.out = nn.Sequential(
            SVDnormalization(ch),
            nn.SiLU(),
            zero_module(SVDconv_nd(dims, model_channels, out_channels, 3, padding=1)),
        )
        if self.predict_codebook_ids:
            self.id_predictor = nn.Sequential(
            SVDnormalization(ch),
            SVDconv_nd(dims, model_channels, n_embed, 1),
        ...
        **kwargs
        ...
        if pool == "adaptive":
            self.out = nn.Sequential(
                ...
                zero_module(SVDconv_nd(dims, ch, out_channels, 1)),
                nn.Flatten(),
            )
        elif pool == "attention":
